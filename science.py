...
def __(IntEnum, TrialData):
    def convert_data(data):
        ...
        return {convert_field_name(field): convert_field_value(field) for field in TrialData.model_fields}
    return convert_data,
...
def __(TrialData, convert_data, pd):
    ...
    raw_df = pd.read_csv("data/data.csv").sort_values(by="average_time_to_finish_sec", ascending=True)
    # Convert to Pydantic data model
    trial_datas = [convert_data(TrialData(**d)) for d in raw_df.to_dict(orient="records")]
...
def __(df, mo):
    ...
    mo.callout(mo.vstack(
        [
            mo.md("# Best Performing Car"),
            mo.ui.table([df.loc[df["Average Time To Finish (sec)"].idxmin()].to_dict()]),
        ]    
    ), kind="success")
    return
...
def __(BodyStyle, ChassisLength, WheelSize, mo):
    ...
    dropdowns = mo.ui.array([
        mo.ui.dropdown(
        options=enum_class.__members__, value=next(iter(enum_class.__members__)), label=enum_name, allow_select_none=False,
...
def __(Path, df, dropdowns, enum_classes, enum_names, mo, np):
    selection_values = [enum_class[dropdown.value] for dropdown, enum_class in zip(dropdowns, enum_classes)]
    ...
    if image_path.is_file():
        ...
        selected_image = mo.callout("No image exists for the selected configuration.", kind="info")

    selected_cimage = mo.center(selected_image)

    mask = np.logical_and.reduce([df[field] == dropdown.value for field, dropdown in zip(enum_names, dropdowns)])
    selected_df = df[mask].head(1)

    selected_table = mo.ui.table(selected_df.reset_index(drop=True))

    mo.vstack([mo.md("# Select-a-Car"), mo.hstack(dropdowns), selected_cimage, selected_table])
    return (
        image_filename,
        image_path,
        mask,
...
def __(mo):
    mo.md(
        ...
        *An mp3 from the nascent internet of the early 2000s.*
        ...
        I have very little information regarding this file. My recollection is that I visited a website that contained a raw `.mid` file, and I recorded the streaming audio output from playing it in Windows Media Player to generate the `.mp3` file here. It was located adjacent to the science project files in my filesystem, and I couldn't resist including it.
...
@app.cell
def __(mo):
    mo.audio(src="assets/tunes/30 Sound.mp3")
    return
